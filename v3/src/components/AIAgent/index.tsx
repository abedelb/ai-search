...
export const AIAgent: React.FC<AIAgentProps> = ({
  ...
  useEffect(() => {
    if (onSlidesUpdate) {
      const allSlides = messages
        .filter(m => m.role === 'assistant' && m.slides && m.slides.length > 0)
        .flatMap(m => m.slides || []);
      onSlidesUpdate(allSlides);
    }
  }, [messages, onSlidesUpdate]);

  const submittedInitialQueryRef = React.useRef<string | null>(null);
  ...
  useEffect(() => {
    if (initialQuery &&
      ...
      const submitInitialQuery = async () => {
        ...
        if (!sessionId) {
          sessionId = await historyService.createChatSession(initialQuery);
        ...
        await sendMessage(sessionId, userMessageId, assistantMessageId, setFeedbackMessageId, filters, initialQuery);
      };
  ...
  }, [initialQuery, messages.length, currentSessionId, filters]);
